INTERFACE zif_zabap_table_edit_config PUBLIC.
  METHODS:
    ...
    change_config DEFAULT IGNORE CHANGING config TYPE zcl_zabap_table_edit=>t_config,
    "! <p class="shorttext synchronized">Called once, e.g. modify commands / hook up other events</p>
    grid_setup DEFAULT IGNORE CHANGING grid TYPE REF TO zif_zabap_table_edit_grid_if,
    "! <p class="shorttext synchronized" lang="en">Change initial fields displayed when user clicks selection</p>
    change_init_selection_fields DEFAULT IGNORE CHANGING fields_tab TYPE zcl_zabap_table_edit_restr_sel=>tt_fields_tab.
ENDINTERFACE.
